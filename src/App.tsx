...
export default function App() {
  ...
  const { user, data, loading, loginWithGoogle, loginWithEmail, registerWithEmail, logout, updateData } = useWorkspace(workspaceId);
  ...
  const [activeModal, setActiveModal] = useState<'projects' | 'locations' | 'import' | 'auth' | null>(null);
    ...
    <div className="min-h-screen bg-stone-50 font-sans text-stone-900 pb-20">
      {!loading && !user && (
        <div className="bg-red-500 text-white text-sm font-medium p-3 text-center shadow-lg sticky top-0 z-[100] flex items-center justify-center gap-3">
          ...
          <button onClick={() => setActiveModal('auth')} className="bg-white text-red-600 px-3 py-1 rounded font-bold hover:bg-red-50 transition drop-shadow-sm">
            Войти (Google / Email)
          </button>
        </div>
      )}
      {/* --- MODALS --- */}
      {activeModal === 'auth' && (
        <div className="fixed inset-0 z-[200] bg-stone-900/40 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col p-6">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold">{authMode === 'login' ? 'Вход' : 'Регистрация'}</h2>
              {user && (
            ...
            <form onSubmit={handleEmailAuth} className="flex flex-col gap-4">
              {authError && <div className="text-red-600 bg-red-50 p-2 text-sm rounded">{authError}</div>}
              <div>
                ...
                <input 
                  ...
                  className="w-full border border-stone-300 rounded p-2 focus:ring-2 focus:ring-blue-500 outline-none" 
                  required 
                />
              </div>
              <div>
              ...
              <button type="submit" className="w-full bg-blue-600 text-white font-bold py-2 rounded shadow-sm hover:bg-blue-700 mt-2">
            ...
            <div className="relative flex items-center my-6">
              ...
              <span className="flex-shrink-0 mx-4 text-stone-400 text-sm">или</span>
              <div className="flex-grow border-t border-stone-200"></div>
            </div>

            <button onClick={() => { loginWithGoogle(); setActiveModal(null); }} className="w-full bg-white border border-stone-300 text-stone-700 font-bold py-2 rounded shadow-sm hover:bg-stone-50 flex items-center justify-center gap-2">
      ...
      <header className="bg-white border-b border-stone-200 px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4 sticky top-0 z-50">
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 w-full md:w-auto">
          ...
          {user && (
            <button 
              onClick={logout}
              className="flex items-center gap-2 bg-stone-200 hover:bg-stone-300 text-stone-700 px-4 py-2 rounded-lg text-sm font-semibold shadow-sm transition-all active:scale-95 ml-2"
              title="Выйти"
            >
              Выйти
            </button>
